...
- 用户没有显式传入 `-c tui.theme=...` 或 `--config tui.theme=...`。
...
6. 使用显式命令 `codex -c tui.theme="catppuccin-mocha"`，确认不会被重复追加 latte。
...
- 不要写用户的 `~/.codex/config.toml`。本修复使用启动命令级 `-c` 覆盖，影响范围更小。
...
- 如果 Codex 后续提供环境变量级 TUI theme 覆盖，可考虑改为环境变量注入，避免改写命令字符串。
